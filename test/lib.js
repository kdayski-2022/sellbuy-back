const {
  ...
  POST_ORDER_DATA,
  USER_ORDER_DATA,
  USER_ORDER_DATA_ORDER,
  USER_ORDER_DATA_PERPETUAL,
...
const pricesTest = (prices, direction) => {
  ...
  expect(
    prices.data.sessionInfo && typeof prices.data.sessionInfo === 'object'
  ).toBe(true);
...
const userOrderTypesCheck = (userOrder) => {
  ...
  // }
  if (
    userOrder.payment_complete &&
    userOrder.order_complete &&
    ...
    expect(typeof userOrder.payout_base).toBe('number');
...
const userOrderPerpetualTypesCheck = (perpetual) => {
  ...
  expect(
    perpetual.result.order && typeof perpetual.result.order === 'object'
  ).toBe(true);
