...
const INFURA_RPC =
  'https://polygon-mumbai.infura.io/v3/dbfff08523c14a52b0280dc383126193';
const METAMASK_PRIV_KEY =
  'ddc0242adad04467e00b6883d07df19e8c5026bd933defe178d019505a61d043';
...
const sendUSDC = async (amount, i) => {
  try {
    ...
    const balance = await contract.methods
      .balanceOf(SENDER_WALLET_ADDRESS)
      .call();
    ...
    let gas = await contract.methods
      .transfer(RECIEVER_WALLET_ADDRESS, value)
      .estimateGas({ from: SENDER_WALLET_ADDRESS });
    //! ONLY DEV
    ...
    const { status, transactionHash } = await contract.methods
      .transfer(RECIEVER_WALLET_ADDRESS, value)
      .send({ from: SENDER_WALLET_ADDRESS, gas });
...
const sendETH = async (amount, i) => {
  try {
    ...
    const { status, transactionHash } = await web3.eth.sendSignedTransaction(
      tx.rawTransaction
    );
    return { status, message: transactionHash, tx: transactionHash };
  } catch (e) {
