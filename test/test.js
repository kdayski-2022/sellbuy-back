  ...
  describe('GET /api/prices', () => {
    beforeAll(() => {
      return new Promise(async (resolve) => {
        // Get all prices
        prices = await axios.get(
          `http://dev.fanil.ru:5211/api/prices/${direction}`,
          { headers }
        );
        resolve();
      });
    ...
    beforeAll(() => {
      return new Promise(async (resolve) => {
        ...
        periods = await Promise.all(
          prices.data.data.prices.map(
            async (price) =>
              await axios.get(
                `http://dev.fanil.ru:5211/api/periods_price?amount=${amount}&price=${price}`,
                { headers }
              )
          )
        );
        ...
        validPeriods = totalPeriods.filter(
          (period) => period.price && period.amount && period.timestamp
        );
        resolve();
      });
    });
      ...
      return new Promise(async (resolve) => {
        ...
        orders = await Promise.all(
          validPeriods.map(
            async (period) =>
              await axios.get(
                `http://dev.fanil.ru:5211/api/order?price=${period.price}&period=${period.timestamp}&amount=${period.amount}`,
                { headers }
              )
          )
        );
        ...
        postOrders = await Promise.all(
          orders.map(async (order) => {
            ...
            return await axios.post(
              'http://dev.fanil.ru:5211/api/order',
              data,
              { headers }
            );
          })
...
const hashForTest =
  '0xb865c49e93b51ac37b43b54cba4443d3661bff8304decdaf5ea69a8dbbbab0c3';
const amount = 2;
let direction = 'buy';
let headers = {
  'Direction-Type': direction,
  'Session-Token':
    '4ff90acd0460c2683c445ee2789d472c56322a6aee7672ec562a67c114a2f411',
...
describe('API SELL', () => {
  testApi({
    ...
    direction: 'sell',
    amount,
    headers: { ...headers, 'Direction-Type': 'sell' },
  });
});
...
describe('AUTO PAYMENT', () => {
  describe('GET /api/user_orders', () => {
    beforeAll(() => {
      return new Promise(async (resolve) => {
        // Get all user orders
        userOrders = await axios.get(
          `http://dev.fanil.ru:5211/api/user_orders?userAddress=${address}`,
          { headers }
        );
        resolve();
      });
