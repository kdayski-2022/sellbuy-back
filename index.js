...
app.get('/test', async (req, res) => {
   const result = axios.get(`${apiUrl}/get_book_summary_by_currency?currency=ETH&kind=option`).then(res =>{
            const fillteredDate = res.data.result.filter(item =>{
...
app.get('/get_prices', async (req, res) => {
    ...
    var currentPrice // TODO Получить цену текущую
        var prices = []
        jsonData.forEach(element => {
             currentPrice = element.underlying_price
        });
