...
db.connection
  ...
  .then(async () => {
    app
      .listen(SECURE_PORT, async () => {
        new Socket();
        app.listen(PORT, async () => {
          console.log(`listen api port ${PORT}`);
              ...
              let orders = await db.models.Order.findAll({
                where: {
                  [db.Op.and]: [
                    ...
                    {
                      status: {
                        [db.Op.notIn]: ['approved', 'denied', 'broken'],
                      },
                    },
                  ],
                },
      ...
      .on('error', (e) =>
        console.error(
          'SECURE_PORT is already in use. Be careful trying to run 2 back instance'
        )
      );
  })
