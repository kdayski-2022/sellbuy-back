...
const writeLog = async (fields) => {
	try {
		...
		const ipAddres = req ? req.headers['x-forwarded-for'] || req.socket.remoteAddress : ''
		const requestParams = req ? JSON.stringify({ ...req.query, ...req.params, ...req.body }) : ''
		...
		const { id } = await db.models.Log.create({ ...fields, userAgent, userAddress, sessionToken, ipAddres, browser, typeMobile, requestParams })
		return id
	} catch(e) {
		throw new Error(e)
	}
...
module.exports = { writeLog, updateLog, destroyLog }