...
const apiUrl =
  process.env.DB_ENV === 'production'
    ? 'https://api.tymio.com/api'
    : 'http://dev2.fanil.ru:5211/api';
...
const getDealExpirationBody = (subscription, order) => {
  try {
    let html;
    ...
    const footer = getDealFooter(subscription, 'expiration');
...
const getDealInitiationBody = (subscription, order) => {
  try {
    let html;
    ...
    const footer = getDealFooter(subscription, 'initiation');
    html = header + body + footer;
...
const getDealFooter = (subscription, type) => {
  return `<tr>
    <td
      style="
        vertical-align: top;
...

{
  ...
  "
  target="_blank"
  href="${apiUrl}/unsubscribe/${subscription.address_hash}">
    Unsubscribe
  </a> */
}
