...
const { convertFloatToBnString } = require('./lib');
...
Eth.getTokenBalance = async function getTokenBalance() {
  return new Promise(async (resolve, reject) => {
    try {
    } catch (error) {}
    const url = `${API_URLS[chain_id]}/api?apikey=${API_KEYS[chain_id]}&`;
    const res = await axios.get(
      `${url}&module=account&action=tokenbalance&address=${serviceAddress}&contractaddress=${usdcTokenAddress}&tag=latest`
    );
    resolve(parseFloat((res.data.result / 1000000).toFixed(2)));
  });
};

Eth.usdcToWei = async (amount, chain_id) => {
  try {
    const web3 = new Web3(
    ...
    const contract = new web3.eth.Contract(
      ERC20Abi,
      WITHDRAWAL_TOKEN_ADDRESS[chain_id],
      { from: WITHDRAWAL_TOKEN_ADDRESS[chain_id] }
    );
...
Eth.usdcFromWei = async (amount, tokenAddress, chain_id) => {
  try {
    const web3 = new Web3(
      new Web3.providers.HttpProvider(INFURA_PROVIDERS[chain_id])
    );
