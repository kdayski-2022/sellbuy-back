...
const { DECIMALS, VALID_AMOUNT } = require('../config/network');
...
const postOrder = async (data) => {
  ...
  postData.params.amount = Math.floor(Number(amount));
  const depositAmount =
...
const calculatePayouts = async (order) => {
  try {
    ...
    if (!order_executed && direction === 'sell') payout_currency = token_symbol;
    if (order_executed && direction === 'buy') payout_currency = token_symbol;
...
const getPayin = async (web3, order) => {
  try {
    ...
    if (data.input !== '0x') {
      ...
      value = await Eth.usdcFromWei(res['1'], data.to, order.chain_id);
    }
...
const getContractHtml = (order) => {
  ...
  let message;
  if (order.direction === 'buy') {
    message = `You are going to lock ${lock} USDC for ${untilExpirationDays} to receive either <span style="color: #627EEA; font-weight: bold">${recieveUSDC}</span> USDC (${USDC} + ${floorRecieve}) if on the ${expirationDate} 8:00 UTC ${
      order.token_symbol
    } price will be <span style="color: #627EEA; font-weight: bold">above</span> <span style="color: rgb(187, 187, 188); font-weight: bold">${
      order.price
    }</span>, or <span style="color: #627EEA; font-weight: bold">${recieveETH} ${
      order.token_symbol
    }</span> (${order.amount} + ${smartRound(
      parseFloat(order.recieve) / parseFloat(order.start_index_price)
    )}) if ${
      order.token_symbol
    } price will be <span style="color: #627EEA; font-weight: bold">below</span> <span style="color: rgb(187, 187, 188); font-weight: bold">${
      order.price
  ...
  if (order.direction === 'sell') {
    message = `You are going to lock ${order.amount} ${
      order.token_symbol
    } for ${untilExpirationDays} to receive either <span style="color: #627EEA; font-weight: bold">${recieveETH}</span> ${
      order.token_symbol
    } (${order.amount} + ${smartRound(
      parseFloat(order.recieve) / parseFloat(order.start_index_price)
    )}) if on the ${expirationDate} 8:00 UTC ${
      ...
      order.price
    }</span>, or <span style="color: #627EEA; font-weight: bold">${recieveUSDC} USDC</span> (${USDC} + ${floorRecieve}) if ${
      order.token_symbol
    } price will be <span style="color: #627EEA; font-weight: bold">above</span> <span style="color: rgb(187, 187, 188); font-weight: bold">${
      order.price
    }</span>. Funds will be returned to your wallet automatically before ${expirationDate} 9:00 UTC.`;
