...
const { checkFirstTx, createAmountEarned, getUserInfo } = require('./user');
const { CHAT } = require('../enum/enum');
const { getCurrentPrice } = require('./price');
dotenv.config();
const apiUrl = process.env.API_URL;

const checkAttemptDuplicates = async (attempt_id) => {
  const orderDuplicates = await db.models.Order.findAll({
    where: { attempt_id },
  });
  if (orderDuplicates.length) {
    telegram.send(`Order with this attempt id ${attempt_id} already exists`);
    return true;
  }
  return false;
};

const checkHashDuplicates = async (hash) => {
  const orderDuplicates = await db.models.Order.findAll({
...
const postOrder = async (data) => {
  ...
  try {
    const isAttemptDuplicate = await checkAttemptDuplicates(attempt_id);
    if (isAttemptDuplicate)
      return {
        success: false,
        error: `Order with this attempt id ${attempt_id} already exists`,
      };
    const isHashDuplicate = await checkHashDuplicates(hash);
    if (isHashDuplicate)
      return {
        success: false,
        error: `Order with this hash ${hash} already exists`,
      };
    ...
    const welcomeBonus = user && parent && !ordersCount;
    const first_tx = ordersCount === 0;
    if (welcomeBonus) {
    ...
    if (status) {
      ...
      try {
        ...
        const message = `TIME-LIMIT ORDER ${direction.toUpperCase()} ${amount} ${token_symbol} ${price} ${formattedDate} APR ${apr.toFixed(
          0
        )}%`;
        await telegram.send(message, CHAT.CLUB);
        await telegram.send(message, CHAT.CLUB_ENG);
      } catch (e) {
      ...
      try {
        ...
        const refUseId = user[0].ref_user_id;
        if (refUseId !== null) {
          const userRef = await db.models.User.findAll({
            where: { id: refUseId },
          });
          const addressRef = userRef[0].address;

          const message = `[CNT: ${
            userOrders.length + 1
          } | TTV: $${totalTradedValue.toFixed(
            0
          )}]\nUser ${address} deposited ${
            Math.round(depositAmount * 100) / 100
          } ${depositToken}\nAmbassador: ${addressRef}`;

          await telegram.send(message, CHAT.AMBASSADOR);
        }
      } catch (e) {
        console.log('While sending message to club error');
        console.log(e);
