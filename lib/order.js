...
const { getSubject, getText, sendMail,parseTransactionDetails } = require('./email');
...
const postOrder = async (data) => {
  ...
  try {
    ...
    if (status) {
      ...
      if (subscription) {
        const subject = getSubject('transaction_notifications', orderDB);
        // const text = getText(orderDB, { start_index_price });
        let html = parseTransactionDetails(orderDB, orderDB.user_payment_tx_hash);
        html = 'You have made a TYMIO transaction, please find details below.<br>' + html
