...
const CLUB_ENG_CHAT_ID = process.env.TELEGRAM_CLUB_ENG_CHAT_ID;
const AMBASSADOR_CHAT_ID = process.env.TELEGRAM_AMBASSADOR_CHAT_ID;
...
function Telegram() {
  ...
  this.clubEngChatId = CLUB_ENG_CHAT_ID;
  this.ambassadorChatId = AMBASSADOR_CHAT_ID;
...
Telegram.prototype.send = async function send(msg, chat = 'logs') {
  try {
    ...
    if (chat === 'ambassador')
      await this.bot.telegram.sendMessage(this.ambassadorChatId, msg, {
        disable_notification: true,
      });
    if (chat === 'club_eng')
      await this.bot.telegram.sendMessage(this.clubEngChatId, msg, {
        disable_notification: true,
      });
  } catch (e) {
    console.log('On sending telegram message error');
