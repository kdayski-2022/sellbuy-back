...
const getFutureTimestamp = (chooseableDay) => {
  ...
  switch (chooseableDay) {
    case TOMORROW:
      const nextDayofWeek = new Date(day.setDate(day.getDate() + 1)).setHours(11, 0, 10, 0);
      return nextDayofWeek;

    case WEEK:
      weeksForward = day.getDay() === 4 ? 1 : 0
      const week = new Date(day.setDate((day.getDate() + ((7 - day.getDay()) % 7 + 5) % 7) + 7 * weeksForward)).setHours(11, 0, 10, 0);
      return week;

    case TWO_WEEK:
      weeksForward = day.getDay() === 4 ? 2 : 1
      const twoWeek = new Date(day.setDate((day.getDate() + ((7 - day.getDay()) % 7 + 5) % 7) + 7 * weeksForward)).setHours(11, 0, 10, 0);
      return twoWeek;

    case MONTH:
      weeksForward = day.getDay() === 4 ? 3 : 2
      const month = new Date(day.setDate((day.getDate() + ((7 - day.getDay()) % 7 + 5) % 7) + 7 * weeksForward)).setHours(11, 0, 10, 0);
      return month;
