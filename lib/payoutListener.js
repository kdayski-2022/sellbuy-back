...
const { CHAIN_LIST, DECIMALS, TOKEN_ADDRESS } = require('../config/network');
...
const listenForPayout = async (orders) => {
  ...
  try {
    const end_index_price = await fetchIndexPrice();
...
// Create payout data
const createPayoutData = async (orders, end_index_price, logId) => {
  try {
    const payoutData = {};
    for (const chain_id of CHAIN_LIST) {
      payoutData[chain_id] = [[], [], []];
